...
Lieferanten-spezifische Primärdaten von den Top-20-Lieferanten erheben und in den Emissionsfaktoren-Katalog überführen, sodass Scope-3-Kat.-1 von spend-based auf supplier-specific umgestellt werden kann.
...
| 5 | EFs in Katalog überführen (`#stahl-supplier-specific` etc.) | Tobias | ⏳ |
...
ERP-Spend + Lieferanten-Fragebogen → Validierung → `carbon-data/emission-factors-catalog.yaml` → `carbon-data/inventory/` → Reporting (ESRS E1-6).
